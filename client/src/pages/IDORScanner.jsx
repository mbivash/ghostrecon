...
function generateSessionId() {
  return (
    "idor-" + Math.random().toString(36).slice(2, 10) + Date.now().toString(36)
  );
}
...
export default function IDORScanner() {
  ...
  const handleScan = async () => {
    ...
    if (!user1.username || !user1.password)
      return setError("Account 1 credentials required.");
    if (!user2.username || !user2.password)
      return setError("Account 2 credentials required.");
    if (user1.username === user2.username)
      return setError("Account 1 and Account 2 must be different users.");
    ...
    const evtSource = new EventSource(
      `${apiBase}/api/idorscan/progress/${sessionId}`,
    );
    ...
    <div style={{ padding: "32px", maxWidth: "1100px" }}>
      ...
      <div style={{ marginBottom: "28px" }}>
        ...
        <p style={{ fontSize: "13px", color: "#555", marginTop: "4px" }}>
          Two-account tester — logs in as two different users and checks if
          Account B can access Account A's data. Best tool for finding broken
          access control in fintech apps.
        </p>
      </div>
      ...
      >
        Only use with two test accounts you own on systems you have written
        permission to test.
      </div>
      ...
      >
        <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          ...
          <Input
            label="Login URL"
            hint="(optional — leave blank to auto-detect)"
          ...
          <div>
            ...
            <div style={{ fontSize: "11px", color: "#444", marginTop: "6px" }}>
              {authType === "cookie"
                ? "For traditional web apps with form-based login"
                : "For React/mobile apps that return a JWT token on login"}
            </div>
          </div>
                ...
                <div>
                  ...
                  <div style={{ position: "relative" }}>
                    <input
                      ...
                      onChange={(e) =>
                        setUser2({ ...user2, password: e.target.value })
                      }
                      style={{
                        width: "100%",
                        boxSizing: "border-box",
          ...
          <div>
            ...
            >
              ...
              <span style={{ color: "#444" }}>(optional — one per line)</span>
            </label>
            <textarea
              placeholder={`https://api.syfe.com/api/v1/portfolio/12345\nhttps://api.syfe.com/api/v1/orders/67890`}
            ...
            <div style={{ fontSize: "11px", color: "#444", marginTop: "4px" }}>
              Add specific API endpoints with IDs you want to test. Tool will
              also auto-discover endpoints.
            </div>
          </div>
          ...
          >
            ...
            >
              I confirm both accounts are mine and I have{" "}
              <span style={{ color: "#a89ff5" }}>written authorization</span> to
              test this application for vulnerabilities.
            </span>
          </label>
          ...
          >
            ...
            <div style={{ height: "294px", overflowY: "auto", padding: "8px" }}>
              {liveFindings.length === 0 ? (
                ...
                >
                  IDOR findings will appear here...
                </div>
              ) : (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          ...
          >
            {[results.sessionA, results.sessionB].map((session, i) => (
              <div
                ...
                style={{
                  background: "#131315",
                  border: `0.5px solid ${session?.authenticated ? "#085041" : "#791F1F"}`,
                  borderRadius: "10px",
                  padding: "14px 18px",
                  display: "flex",
                  alignItems: "center",
              ...
              >
                <div
                  style={{
                    ...
                    height: "8px",
                    borderRadius: "50%",
                    background: session?.authenticated ? "#1D9E75" : "#E24B4A",
                    flexShrink: 0,
                  }}
                />
                <div>
                  <div
                    style={{
                      fontSize: "13px",
                      color: session?.authenticated ? "#1D9E75" : "#E24B4A",
                      fontWeight: "500",
                    }}
                  >
                    Account {i === 0 ? "A" : "B"} —{" "}
                    {session?.authenticated ? "Authenticated" : "Failed"}
                  </div>
          ...
          >
            {[
              {
                label: "IDOR findings",
                val: results.summary?.total || 0,
                color: results.summary?.total > 0 ? "#ff4444" : "#1D9E75",
              },
              {
                label: "High severity",
                val: results.summary?.high || 0,
                color: "#E24B4A",
              },
              {
                label: "Endpoints tested",
                val: results.summary?.testedEndpoints || 0,
                color: "#7F77DD",
              },
              {
                label: "APIs discovered",
                val: results.summary?.apiEndpointsFound || 0,
                color: "#a89ff5",
              },
            ].map((s) => (
              <div
            ...
            {results.findings.length === 0 ? (
              <div style={{ padding: "32px", textAlign: "center" }}>
                ...
                <div style={{ color: "#444", fontSize: "12px" }}>
                  Try adding custom API endpoints manually in the form above
                </div>
              </div>
            ) : (
